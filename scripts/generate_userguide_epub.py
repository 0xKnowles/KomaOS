...
OUTPUT_EPUB = ROOT / "KomaOS_User_Guide.epub"
...
def split_chapters(html: str):
    ...
    if not positions:
        return [('intro', 'KomaOS User Guide', html)]
...
def build_epub():
    ...
    book = epub.EpubBook()
    book.set_identifier('komaos-user-guide-v1')
    book.set_title('KomaOS User Guide')
    book.set_language('en')
    book.add_author('KomaOS Project')
    ...
    # Full-size cover image (480×800) — used by KomaOS for home-screen thumbnail.
    ...
    # EPUB 2 cover declaration — KomaOS's Tier 1 OPF lookup
    book.add_metadata('OPF', 'meta', '', {'name': 'cover', 'content': 'cover-image'})

    # Cover page XHTML — first readable page in the spine
    cover_page = epub.EpubHtml(title='KomaOS User Guide', file_name='cover.xhtml', lang='en')
    cover_page.content = make_xhtml(
        'KomaOS User Guide',
        '<div class="cover-page">'
        '<img src="images/cover.png" alt="KomaOS User Guide cover"/>'
        '</div>',
    )
    ...
    # nav is excluded from the spine — KomaOS locates it via properties="nav" in
