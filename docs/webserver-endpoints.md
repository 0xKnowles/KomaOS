...
available while KomaOS is in File Transfer or Calibre Wireless mode.
...
Examples use `komaos.local`. If mDNS does not resolve on your network, use
...
```bash
curl http://komaos.local/api/status
```
...
```bash
curl "http://komaos.local/api/files?path=/Books"
```
...
```bash
curl -OJ "http://komaos.local/download?path=/Books/MyBook.epub"
```
...
```bash
curl -X POST -F "file=@mybook.epub" "http://komaos.local/upload?path=/Books"
```
...
```bash
curl -X POST -d "name=NewFolder&path=/" http://komaos.local/mkdir
```
...
```bash
curl -X POST -d "path=/Books/old.epub&name=new.epub" http://komaos.local/rename
```
...
```bash
curl -X POST -d "path=/Books/mybook.epub&dest=/Read" http://komaos.local/move
```
...
```bash
curl -X POST -d "path=/Books/mybook.epub" http://komaos.local/delete
curl -X POST -d 'paths=["/Books/old.epub","/OldFolder"]' http://komaos.local/delete
```
...
```bash
curl http://komaos.local/api/settings
```
...
curl -X POST \
  ...
  http://komaos.local/api/settings
```
...
```bash
curl http://komaos.local/api/fonts
```
...
curl -X POST \
  ...
  http://komaos.local/api/fonts/upload
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"family":"Literata"}' \
  http://komaos.local/api/fonts/delete
```
...
```bash
curl http://komaos.local/api/opds
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"index":0}' \
  http://komaos.local/api/opds/delete
```
...
```bash
curl http://komaos.local/api/wifi
```
...
curl -X POST \
  -H "Content-Type: application/json" \
  -d '{"index":0}' \
  http://komaos.local/api/wifi/delete
```
...
- `komaos.local` is advertised with mDNS when available.
- `/api/status` returns `"mode": "STA"` and RSSI in dBm.

### Access Point Mode (AP)

- Device creates an open hotspot named `KomaOS-Reader`.
