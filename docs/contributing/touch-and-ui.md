...
KomaOS now runs on touch devices (Seeed Sticky, M5Paper, LilyGo T5) alongside the button-only Xteink X3/X4. Every screen must work with both input styles. There are two supported ways to get there:
...
build_flags =
  ...
  -DBOARD_HAS_PSRAM
  -DKOMAOS_VERSION=\"${komaos.version}-m5paper\"
  -DENABLE_SERIAL_LOG
  -DLOG_LEVEL=2
  ; touch-first device: hide front-button hint labels
  -DKOMAOS_SHOW_BUTTON_HINTS=0
...
build_flags =
  ...
  -DKOMAOS_VERSION=\"${komaos.version}-lilygo\"
  -DENABLE_SERIAL_LOG
  -DLOG_LEVEL=2
  -DKOMAOS_SHOW_BUTTON_HINTS=0
lib_deps =
