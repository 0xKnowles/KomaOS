...
void BaseTheme::drawHeader(const GfxRenderer& renderer, Rect rect, const char* title, const char* subtitle) const {
  ...
  const bool showBatteryPercentage =
      SETTINGS.hideBatteryPercentage != KomaSettings::HIDE_BATTERY_PERCENTAGE::HIDE_ALWAYS;
...
void BaseTheme::drawStatusBar(GfxRenderer& renderer, const float bookProgress, const int currentPage,
  ...
  if (sb.showsProgressBar()) {
    ...
    size_t progress;
    if (sb.progressBarMode == KomaSettings::STATUS_BAR_PROGRESS_BAR::BOOK_PROGRESS) {
  ...
  if (sb.showsClock() && halClock.isAvailable()) {
    ...
    if (halClock.formatTime(timeBuf, sizeof(timeBuf), sb.clockUtcOffsetQ, sb.clock12h)) {
      ...
      } else if (sb.clockMode == KomaSettings::STATUS_BAR_CLOCK_RIGHT) {
