...
void KomaSettings::validateFrontButtonMapping(KomaSettings& settings) {
...
uint8_t KomaSettings::sleepTimeoutEnumToMinutes(const uint8_t legacyValue) {
  switch (legacyValue) {
    case SLEEP_1_MIN:
      return 1;
    case SLEEP_5_MIN:
...
void KomaSettings::toJson(JsonDocument& doc) const {
  const KomaSettings& s = *this;
...
bool KomaSettings::fromJson(JsonVariantConst doc) {
  KomaSettings& s = *this;
...
KomaSettings::StatusBarSpec KomaSettings::statusBarSpec() const {
  StatusBarSpec spec;
...
float KomaSettings::getReaderLineCompression() const {
...
unsigned long KomaSettings::getSleepTimeoutMs() const {
...
void KomaSettings::clearSdFontFamily() {
...
int KomaSettings::getReaderFontId() const {
