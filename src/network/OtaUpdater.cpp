...
namespace {
constexpr char latestReleaseUrl[] = "https://api.github.com/repos/0xKnowles/KomaOS/releases/latest";
}  // namespace

OtaUpdater::OtaUpdaterError OtaUpdater::checkForUpdate() {
  LOG_DBG("OTA", "Checking for update (current: %s)", KOMAOS_VERSION);
...
bool OtaUpdater::isUpdateNewer() const {
  if (!updateAvailable || latestVersion.empty() || latestVersion == KOMAOS_VERSION) {
    return false;
  }
  ...
  const auto currentVersion = KOMAOS_VERSION;
